...
def get_pupil_performance(path):
    ...
    pupil_df['performance'] = None 

    for i in tqdm(range(len(pupil_df))):
        tesm_sess_trial_ring = pupil_df.iloc[i][['teamID', 'sessionID', 'trialID', 'ringID']]
        try:
            ...
            continue           
        pupil_df.at[i, 'performance'] = performance_df.iloc[performance_id].performance
   
    pupil_df.performance = pd.to_numeric(pupil_df.performance)
    return pupil_df
...
def mixed_effects_model(pupil_ISC_df):
    action_ISC_performance = copy.deepcopy(pupil_ISC_df)
    action_ISC_performance = action_ISC_performance.drop(columns={'yawPupil', 'pitchPupil', 'thrustPupil'})
...
if __name__ == '__main__':
    ...
    pupil_performance_df = get_pupil_performance(path)
