...
class PredictAction(Dataset):
    ...
    def __getitem__(self, idx):
        eeg, pupil, action, location, out_action = self.current_data
        ...
        src1, src2, src3, src4, tgt, trg_y = self.get_src_trg(
        sequence1=input_eeg,
        sequence2=input_pupil,
        sequence3=input_action,
        sequence4=input_location,
        output_seq=output_data)
        
        return src1, src2, src3, src4, tgt, trg_y
    ...
    def get_current_data(self):
        ...
        return eeg, pupil, action, location, out_action
    
    def get_src_trg(self, sequence1: torch.Tensor, sequence2: torch.Tensor, sequence4: torch.Tensor, output_seq: torch.Tensor):
        ...
        # if self.flag == 'train':
        #     tgt = output_seq[self.target_seq_len-1:-1]  # Shifting output_seq
        # else:
        #     tgt = torch.cat((output_seq[self.target_seq_len-1:self.target_seq_len], output_seq[:self.target_seq_len-1]), dim=0)
        ...
        return src1, src2, src3, src4, tgt+1, (trg_y+1).long()